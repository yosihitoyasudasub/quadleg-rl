...
def draw_mech(X0, Y0, S, p, lw=1.0, shank_len=95, labels=True):
    ...
    A('<g transform="translate(%.1f,%.1f) rotate(%.1f)">' % (X0, Y0, ang))
    rect(-4 * S, -58 * S, 8 * S, 66 * S, FB_F, FB, 1.6 * lw, 3)                 # 腕（2 枚が重なって見える）
    rect(-1.5 * S, -55 * S, 3 * S, 15 * S, '#ffffff', FB, 1.2 * lw, 1.5)        # スロット 40〜55
    A('</g>')
    # シャンク（K 側は途中まで）
...
txt(gx(p['C']) + 24, gy(p['C']) - 20, 'スロット 幅 3（丸端 40 / 55）。腕は 2 枚', 10.5, FB, 'start')
...
txt(40, 836, '小リンクが受けるのは足パッドの摩擦（横力）× 100 mm のモーメントだけ（横力 30 N で 50〜70 N。設計は 140 N で見る）。', 11.5, '#333')
...
layers = [
    ('リンク①',   -9.5, -7.5, LK_F, LK,  25,  -4, 30),
    ('腕①',      -7.25, -4.75, FB_F, FB, 34, -12, 38),
...
for z0 in (-7.25, 4.75):
    rect(zx(z0), vy(32), 2.5 * SZ, 16 * SV, '#ffffff', FB, 1.0)             # スロット（v 16〜32）
rect(zx(-10.7), vy(22.5), 21.4 * SZ, 3 * SV, PN_F, PN, 1.3)                    # ピン C（v = 21）
for z in (-10.9, 10.5):
    rect(zx(z) - 0.25 * SZ, vy(24.5), 0.5 * SZ, 7 * SV, PN_F, PN, 1.0)         # E リング
rect(zx(-0.5), vy(1.5), 11 * SZ, 3 * SV, PN_F, PN, 1.3)                         # ピン A2（v = 0、腕は無い位置）
rect(zx(4.25), vy(1.5), 3.25 * SZ, 3 * SV, '#e8eaed', '#9aa0a6', 1.0)           # スペーサ
txt(zx(12.5), vy(23), 'ピン C φ3 × 22', 11, PN, 'start', 'bold')
txt(zx(12.5), vy(19), 'リンク①・腕①・腕②・リンク② を貫く', 10, PN, 'start')
txt(zx(12.5), vy(15.5), '両端 E リング', 10, PN, 'start')
txt(zx(12.5), vy(1), 'ピン A2 φ3 × 12', 11, PN, 'start', 'bold')
txt(zx(12.5), vy(-3), 'シャンク②・スペーサ 3.25・リンク②', 10, PN, 'start')
txt(zx(12.5), vy(-7), 'A1 は鏡像（完全に左右対称）', 10, PN, 'start')
txt(zx(12.5), vy(-10.5), '（腕はこの位置には無い。スペーサは層を保つため）', 10, PN, 'start')
txt(zx(-10.5), vy(-31), 'z = −9.5', 10, '#5b6470', 'start')
txt(zx(9.5), vy(-31), '+9.5 mm', 10, '#5b6470', 'end')
txt(640, vy(-38), 'シャンク①②は §5-1 の足先と同じ層。腕はブロックの二股の耳を両方とも上へ延ばし、小リンクはその外側に置く。',
    10.5, '#333', 'start')
