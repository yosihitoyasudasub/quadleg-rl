...
LS0 = 100.0         # バネ自由長 [mm]（キャリア → 足）。サミニ 11-1437
PAD = 12.0          # 足＋パッドの半径ぶん [mm]
ROD_UP = 212.0      # キャリアからロッド上端まで [mm]（最大 |OF| 199 ＋ ブッシュ半長）
ROD_DN = 4.0        # キャリアからロッド下端まで [mm]（ロッドはキャリアで終わる）
...
txt(ax(p['at'](LS0)) + 24, ay(p['at'](LS0)) + 4, '足（ガイド管の中を滑る）', 11, '#333', 'start')
...
SB = 0.78
for BXc, tipy, lab, sd in ((490.0, -139.0, '屈（|OF| = 139）', -1),
...
txt(420, 534, '赤丸＝ロッド上端。股ブッシュより上へ出る量が 73 → 17 mm と変わり、',
    11.5, '#333')
txt(420, 552, 'その差ぶんだけ股→足が 239 → 295 mm に伸びる。ロッドは股で固定されていない。',
    11.5, '#333')
...
CXp, CY0, SC = 1030.0, 158.0, 2.32
cy = lambda v: CY0 + v * SC        # v = キャリア F からロッドに沿って下向き [mm]
RW = 5.0 * SC / 2                  # ロッド半径 φ5
TI, TO = 7.6 * SC, 8.6 * SC        # ガイド管 内半径 / 外半径（内径 15.2・肉厚 1.0）

# ガイド管（キャリアに固定。ばねの外側ガイド）
for sg in (-1, 1):
    rect(CXp + sg * TI if sg > 0 else CXp - TO, cy(9), TO - TI, 103 * SC, '#ffe8cc', BU, 1.5)
# 伸び切りストッパ（管下端の内向きつば）
for sg in (-1, 1):
    rect(CXp + sg * TI - (2.5 * SC if sg > 0 else 0), cy(108), 2.5 * SC, 4 * SC,
         '#ffe8cc', BU, 1.3)
# ロッド（キャリアより上だけ）
rect(CXp - RW, cy(-20), 2 * RW, 29 * SC, '#dfe3e8', RD, 1.6)
# キャリア（ロッドに固定）
...
# ばね（管の中）
coil(CXp, cy(12), CXp, cy(94), 9, 6.2 * SC, SP, 2.4)
# 足（ピストン。管の中を滑る）＋ゴムパッド
rect(CXp - TI, cy(94), 2 * TI, 16 * SC, '#e8eaed', RD, 1.8, 2)
rect(CXp - 13 * SC, cy(112), 26 * SC, 7 * SC, '#4b4b4b', '#333', 1.3, 2)
ground(CXp, cy(119), 185)
...
lead(-17, -1, 'ロッド φ5', '↑ 股のブッシュへ', RD)
lead(0, 1, 'シャンクのピン F', 'キャリアに 2 本が集まる', SH)
lead(6, -1, 'キャリア', 'ロッドに固定（滑らない）', FB)
lead(32, 1, 'ガイド管（外側ガイド）', 'PA-CF 内径 15.2 × 肉厚 1.0', BU)
lead(52, -1, '底付きストッパ', '縮み 58 mm で当てる', '#5b6470')
lead(66, 1, '圧縮コイルばね', 'サミニ 11-1437（自由長 100）', SP)
lead(100, -1, '足（ピストン）', '管の中を滑る＝ストローク', '#333')
lead(110, 1, '伸び切りストッパ', '管下端の内向きつば', BU)
line(CXp + 104, cy(0), CXp + 17 * SC, cy(0), SH, 1.0)
line(CXp - 104, cy(6), CXp - 17 * SC, cy(4), FB, 1.0)
line(CXp - 104, cy(-17), CXp - RW, cy(-17), RD, 1.0)
line(CXp + 104, cy(32), CXp + TO, cy(32), BU, 1.0)
line(CXp - 104, cy(52), CXp - TO, cy(52), '#5b6470', 1.0)
line(CXp + 104, cy(66), CXp + 16, cy(66), SP, 1.0)
line(CXp - 104, cy(100), CXp - TI, cy(100), '#333', 1.0)
line(CXp + 104, cy(110), CXp + TI, cy(110), BU, 1.0)
...
body = [
    ...
    ('4. 長さ変化はロッドが股ブッシュを滑って吸収する。',
     ['股→足は 239〜295 mm（|OF| 139〜195 ＋ バネ 100）で変わる。'
