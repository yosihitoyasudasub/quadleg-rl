...
# C. 足先ブロックの腕
# ==================================================================
txt(860, 96, 'C. 足先ブロックの腕', 14.5, '#111', 'start', 'bold')
S3 = 4.6
...
txt(860, 560, '厚さ 2.5（ブロックの片方の耳をそのまま上へ延ばす）。', 11, '#333')
txt(860, 576, 'z = +4.75 〜 +7.25 の層。材質はブロックと同じ。質量 1.3 g', 11, '#333')
...
SZ, SV = 13.0, 3.4
LAYERS = [('リンク①', -6.5, -4.5, LK_F, LK, 12, -12),
...
def zsec(X, Y, title, sub, through, spacer=None, slot=False):
    zx = lambda z: X + z * SZ
    vy = lambda v: Y - v * SV
    ...
    if slot:
        rect(zx(4.75), vy(10), 2.5 * SZ, 16 * SV, '#ffffff', FB, 1.0)
    z0, z1 = through
...
zx, vy = zsec(230.0, 725.0, 'ピン A2（A1 は鏡像）', 'シャンク② ＋ スペーサ ＋ リンク②',
              (-0.5, 10.5), spacer=(4.25, 7.5))
lead(zx(5.9), vy(3), zx(5.9) + 26, vy(3) - 42, 'スペーサ 3.25', '#9aa0a6')
txt(zx(5.9) + 30, vy(3) - 26, '（腕の層を飛び越える）', 10, '#9aa0a6')
txt(230.0, 820, 'φ3 × 15。A1 側は腕が無いのでスペーサ不要（φ3 × 12）', 10.5, '#333', 'middle')

zx, vy = zsec(640.0, 725.0, 'ピン C', 'リンク① ＋ 腕（スロット）＋ リンク②', (-8.5, 11.0), slot=True)
for z in (-8.7, 10.8):
    rect(zx(z) - 0.25 * SZ, vy(3.5), 0.5 * SZ, 7 * SV, PN_F, PN, 1.0)
lead(zx(-8.7), vy(5), zx(-8.7) - 14, vy(5) - 38, 'E リング × 2', PN, 10.5, 'end')
txt(640.0, 820, 'φ3 × 22。腕のスロットの中を滑るので、ここだけ締め付けない', 10.5, '#333', 'middle')

# ==================================================================
# まとめ
# ==================================================================
rect(30, 838, W - 60, 156, '#f7f9fb', '#c9d3de', 1.2)
txt(50, 864, '組む順番と確認', 13.5, '#111', 'start', 'bold')
for i, s in enumerate([
    ...
    '   質量: 小リンク 1.54 g × 2 ＋ ピン 3 本 2.74 g ＋ スペーサ 0.18 g ＋ E リング 0.1 g ＝ 6.1 g、'
    '腕 1.3 g。合計 7.4 g が足の質量に加わる。',
]):
    txt(50, 888 + i * 22, s, 11.5, '#333')
txt(50, 982, '※ 精度が効くのは「4 辺が等しいこと」だけ。穴間 30 mm を左右 4 箇所で揃える。'
