...
lead(ax0 - 1.06 * S, ay0 - 1.06 * S, ax0 - 5.5 * S, ay0 - 3.4 * S, 'φ4 H7 × 2', LK, 10.5, 'end')
...
txt(ax0 - 4 * S, by + 4.4 * S + 16, '軸力 140 N → 引張 17.5 MPa、穴の面圧 17.5 MPa（φ4 で計算）', 11, '#333')
...
circ(bx1, by0, 4.5 * S2, '#f6d5b0', SH, 1.4)                     # 局所ボス φ9（穴の周囲、正面図では円で見える）
...
lead(bx1 + 3 * S2, my(3.5), mx(38), my(12), '局所ボス φ9', RED, 10.5, 'start')
txt(mx(38) + 2, my(12) + 15, '（A1/A2 の φ4 H7 はこの中）', 10, RED, 'start')
...
# --- B 側面図（局所ボスの厚みを見せる。text3 の下に十分な間隔を空けて配置）---
title_y = 305.0
bby = title_y + 3.25 * S2 + 12.0
txt(bx0 - 20 * S2, title_y, 'B 側面図（ボスは外面側だけに盛る。板厚 2.5 → 5.75 mm）', 10.5, DIM)
rect(bx0 - 20 * S2, bby, 60 * S2, 2.5 * S2, SH_F, SH, 1.6)                          # 主スパン t2.5
rect(bx1 - 4.5 * S2, bby - 3.25 * S2, 9 * S2, 3.25 * S2 + 2.5 * S2, SH_F, SH, 1.6)  # 局所ボス（外面側 +3.25）
cl(bx1, bby - 4 * S2, bx1, bby + 4.5 * S2)
dimv(bby - 3.25 * S2, bby, bx1 + 5.5 * S2, '+3.25', bx1 + 4.5 * S2, 1)
dimv(bby, bby + 2.5 * S2, bx1 + 5.5 * S2, 't2.5', bx1 + 4.5 * S2, 1)
txt(bx0 - 20 * S2, bby + 6.5 * S2,
    'ボス高さ 3.25 mm は小リンクとの隙間（旧スペーサ寸法）に一致 → 別部品のスペーサが不要になる', 10.5, '#333')
...
L_A = [('シャンク②', 0.25, 4.25, SH_F, SH, 6, -18),
       ('', 4.25, 7.5, '#f6d5b0', SH, 6, -18),           # 局所ボス。幅が狭いので下に引出線で別途ラベル
       ('リンク②', 7.5, 9.5, LK_F, LK, 12, -12)]


def zsec(X, Y, title, sub, layers, through, press_to=None, slots=()):
    zx = lambda z: X + z * SZ
    vy = lambda v: Y - v * SV
    ...
    if press_to is not None:
        # 圧入部だけハッチングを掛けて示す（z1 側＝ボス+シャンクの奥まで、貫通させない）
        hz0, hz1 = press_to
        for hx in range(int(zx(hz0)), int(zx(hz1)), 3):
            line(hx, vy(1.5), hx + 3, vy(-1.5), PN, 0.7)
    return zx, vy
...
zx, vy = zsec(880.0, 725.0, 'ピン A2（A1 は鏡像）', 'シャンク② ＋ 局所ボス（圧入）＋ リンク②',
              L_A, (0.25, 10.5), press_to=(0.25, 7.5))
for z in (10.7,):
    rect(zx(z) - 0.25 * SZ, vy(3.5), 0.5 * SZ, 7 * SV, PN_F, PN, 1.0)
lead(zx(10.9), vy(5), zx(10.9) + 10, vy(5) - 40, 'E リング（片端のみ）', PN, 10.5, 'start')
lead(zx(4), vy(-2), zx(4) - 30, vy(-2) - 30, '圧入部（斜線）', PN, 10.5, 'end')
lead(zx(5.9), vy(-2), zx(5.9) + 55, vy(-2) + 4, '局所ボス', SH, 10.5, 'start')
txt(880.0, 820, 'φ4 h6 ＋圧入（H7/n6）。ブラインドで z=0.25 止まり（シャンク①との', 10.5, '#333', 'middle')
txt(880.0, 836, '0.5 mm 隙間には出さない）。ボス高さ 3.25 はスペーサ寸法と同じで別部品不要。A1 側も同じ。', 10.5, '#333', 'middle')
...
for i, s in enumerate([
    '1. シャンク①② に φ4 H7 の穴（F から 30）＋局所ボス φ9 を追加。2. 小リンクを 2 枚作る（穴間 30、φ4 H7）。'
    '3. 足先ブロックに腕とスロットを付ける。4. ジョイントを組む。',
    '   A1・A2 は φ4 圧入ピン（H7/n6、ブラインド）＋小リンク側 E リング。緩み対策で段付ボルトから変更（2026-09-20）。'
    '腕と C は「ピン-スロット」（スライダにすると過拘束になる）。',
    '   検算: 菱形は対辺が平行なので、リンク①（A1→C）は常にシャンク②（F→K2）と平行。'
    '全姿勢で差 0.000°。Fusion の計測で確かめられる。',
    '   質量: 小リンク 1.54×2 ＋ 腕 1.3×2 ＋ ピン C 1.23 ＋ ピン A（φ4）1.19×2 ＋ E リング 0.2 ＋ ボス 0.58×2 ＝ '
    '9.7 g が足の質量に加わる（局所ボスはシャンク重量に含めてもよい）。',
]):
