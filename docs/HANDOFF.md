...
**2026-09-06 に四節リンクから 5 節リンクへ変更。** 股関節 O に 2 本のクランク（長さ l1）が同軸で付き、
それぞれの先から下腿（長さ l2）が伸びて足先で合流する。膝は受動関節で、**脚にサーボが載らない**。
...
**確定値（1.0 kg 前提で検証済み）**: **l1 = 30 mm（クランク）、l2 = 65 mm（下腿）**、立ち高さ 67 mm

| 項目 | 値 |
| --- | --- |
| 立ち姿勢のクランク角 | **θ1 = −163.2°（後側） / θ2 = −16.8°（前側）** |
| 立ち姿勢の内角（下腿どうし） | 52° |
| 足先の到達範囲 | 股から **35〜95 mm**（|l1−l2| 〜 l1+l2） |
| 実用可動域 | 前後 ±25 mm、高さ −40〜−85 mm（この範囲で内角 28° 以上） |
| クランク角の可動幅 | 約 105°（XL-320 の 300° に十分収まる） |

**トルク（1.0 kg、2 脚支持、動的係数 1.5、XL-320 ストール 0.390 / 連続 0.130 N·m）**

| 足先 X | サーボ①（後） | サーボ②（前） |
| --- | --- | --- |
| −25 mm | 0.206（53%） | 0.022 |
| 0（立位） | 0.121 | 0.121 |
| +25 mm | 0.022 | 0.206（53%） |

前後に振ると片方が主に受け持つ形になり、**最大でもストールの 53%**。連続定格 0.130 に対しては立位で 93%
だが、トロットではデューティ 50% 程度なので許容範囲。
...
`{"l1":30,"l2":65,"fx":0,"fy":-67,"mass":1.0,"legs":2,"sf":1.5,"fxr":0,"servo":0,"stall":4.0,"sW":24,"sH":36,"sOff":9,"sA":90,"oy":0,"oz":0,"t3":0,"bodyW":110,"bodyL":140}`
