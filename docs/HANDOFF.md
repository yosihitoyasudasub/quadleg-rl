...
- **確定値（2026-09-06、質量 1.0 kg 前提で検証済み）**: **L1=48→44, L2=44, Lc=6, Le=12, Lr=44 mm**, δ=0, cfg=A
  - 立ち姿勢 **θ1 = −130.5°, θ2 = 125.5°** → 足先 (0, −66.9) mm（立ち高さ 67 mm）、膝角 98.9°、伝達角 88.9°
  - Lc=6 はホーン穴 P.C.D 12（半径 6）にロッドを直付け。クランク:膝 = 1:2 の減速
  - トルク（1.0 kg、動的係数 1.5、XL-320 ストール 0.390 / 連続 0.130 N·m）

    | 状況 | 股 | 膝 | 備考 |
    | --- | --- | --- | --- |
    | 静止（4 脚支持） | 0.052 | 0.052 | 連続定格の 40% |
    | 立位（2 脚支持） | 0.104 | 0.105 | 連続定格の 80% |
    | 歩行ピーク（足先 ±25 mm） | **0.220（ストールの 56%）** | 0.125（32%） | ここが一番厳しい |
    | ロッド軸力 | 17.5 N（立位） | | Ø3 ロッドで十分 |
...
- Lc を自由に選べる場合（ホーンにアダプタ円板を付ける場合）は **Lc=8 / Le=16** が最適。
  サーボトルクは比 Lc:Le だけで決まるため変わらないが、ロッド軸力が 17→13 N に下がり、
  ピン遊び 0.1 mm あたりの膝角誤差が 0.48° → 0.36° に改善する（機構のガタはサーボ分解能 0.15° より支配的）
...
- **現行 params（Web アプリ貼り付け用）**:
  `{"bx":0,"by":0,"L1":44,"L2":44,"Lc":6,"Lr":44,"Le":12,"delta":0,"cfg":1,"t1":-130.5,"t2":125.5,"mass":1.0,"legs":2,"sf":1.5,"fxr":0,"servo":5,"stall":4.0,"sW":24,"sH":36,"sOff":9,"sPCD":12,"sN":8,"sA1":0,"sA2":0,"oy":0,"oz":0,"t3":0,"bodyW":110,"bodyL":140}`
...
XL-320 での余裕（L=44・1.0 kg・上表）: 歩行ピークで股 56%、膝 32%。**設計質量は 1.0 kg**（2026-09-06 決定）。ストールは瞬時値、連続は 1/3 目安（XL-320 なら 0.130 N·m）。1.2 kg を超えると立位で連続定格に達する。
