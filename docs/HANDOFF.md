...
| 一本脚ホッパー 2D シミュレーター | **`tools/webapp/hopper-2d.html`**（2026-09-07 作成、Node で検証済み）。§8 参照 |
...
**次の作業（ホッパー側）**
1. 前進速度: 蹴り出し時間を短くする（ΔL 到達後は保持）／胴体慣性の確定／`cn`・`Kv` の再調整
2. 速度推定・接地検出を実機で得られる量（IMU 積分・バネ圧縮量）に置き換えて再評価
3. ロール軸を含む 3D 化は MuJoCo（MJCF）で行う。2D で成立しなければ 3D に進まない
