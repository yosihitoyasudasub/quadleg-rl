...
- **確定値（2026-09-06、質量 1.0 kg 前提で検証済み）**: **L1=44, L2=44, Lc=8, Le=16, Lr=44 mm**, δ=0, cfg=A
  - 立ち姿勢 **θ1 = −130.5°, θ2 = 126.9°** → 足先 (0, −66.9) mm（立ち高さ 67 mm）、膝角 98.9°、伝達角 88.5°
  - **Lc=8 はホーン穴 P.C.D 12（半径 6）を超えるのでアダプタ円板が必要**。ホーンの 4-Ø4 リベット穴または
    4-Ø1.6 タップ穴（P.C.D 12）で固定し、半径 8 の位置にロッド取付穴を設ける。ここが強度上の弱点になる
  - クランク:膝 = 1:2 の減速（サーボ 1° に対し膝 0.5°、可動 300° で膝 150°）
  - トルク（1.0 kg、動的係数 1.5、XL-320 ストール 0.390 / 連続 0.130 N·m）

    | 状況 | 股 | 膝 | ロッド軸力 | 備考 |
    | --- | --- | --- | --- | --- |
    | 静止（4 脚支持） | 0.052 | 0.053 | 6.6 N | 連続定格の 40% |
    | 立位（2 脚支持） | 0.105 | 0.105 | 13.2 N | 連続定格の 81% |
    | 歩行ピーク（足先 ±25 mm） | **0.221（ストールの 57%）** | 0.128（33%） | 16.9 N | ここが一番厳しい |

  - 伝達角は掃引中の最小で 83°（立位 88.5°）
  - ピン遊び 0.1 mm あたりの膝角誤差 **0.36°**（Le=12 なら 0.48°）。サーボ分解能由来の 0.15° より大きく、
    機構のガタが精度を支配する

  - **学習時は `forcerange = 0.39 N·m` を必ず設定**し、実機で出せないトルク前提の歩容を学習させない
  - **トロット中の外転角は ±15° まで**。20° で外転トルク 0.168 N·m となり連続定格を超える
- 設計値の検証は `tools/leg_torque.py`（Web アプリと同じ τ = −Jᵀ·F）で再現できる
- Lc/Le の選定根拠: サーボトルクは比 Lc:Le だけで決まり、絶対値には依存しない。
  Lc を大きくすると**ロッド軸力が反比例して下がり**、Le を大きくすると**ガタ由来の膝角誤差が下がる**。
  6/12 → 8/16 でロッド 17.5→13.2 N、膝角誤差 0.48→0.36°。
  さらに 10/20 も可能だが、下腿 44 mm に対しレバー 20 mm は張り出しが大きく干渉が増える
- 旧 params（2 自由度時代、L=48・2.2 kg 想定）:
...
- **現行 params（Web アプリ貼り付け用）**:
  `{"bx":0,"by":0,"L1":44,"L2":44,"Lc":8,"Lr":44,"Le":16,"delta":0,"cfg":1,"t1":-130.5,"t2":126.9,"mass":1.0,"legs":2,"sf":1.5,"fxr":0,"servo":5,"stall":4.0,"sW":24,"sH":36,"sOff":9,"sPCD":12,"sN":8,"sA1":0,"sA2":0,"oy":0,"oz":0,"t3":0,"bodyW":110,"bodyL":140}`
- 計算モデル: 静的釣合い τ = −Jᵀ·F（ヤコビアンは数値微分）。リンク自重・摩擦・慣性は未考慮

### サーボ仕様（ROBOTIS 図面・e-Manual で確認済み）
| 型番 | ストール | 質量 | 本体 W×H×D | ホーン軸〜上端 | ホーン | 備考 |
| --- | --- | --- | --- | --- | --- | --- |
| XL-320 | 0.39 N·m @7.4 V (≈4.0 kg·cm) | 16.7 g | 24×36×27 | 9.0 | Ø17.8、4-Ø4 リベット + 4-Ø1.6 タップ、P.C.D 12 | Joint 0–300°、電流制御なし、分解能 0.29°、2S 直結、**Sync Read 可 / Bulk Read 不可**、位置データ 2 バイト |
...
XL-320 での余裕（L=44・1.0 kg・上表）: 歩行ピークで股 57%、膝 33%。**設計質量は 1.0 kg**（2026-09-06 決定）。ストールは瞬時値、連続は 1/3 目安（XL-320 なら 0.130 N·m）。1.2 kg を超えると立位で連続定格に達する。
