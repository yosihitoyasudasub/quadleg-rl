...
**ガイドを外側にする理由**: ばね内径が 10.8 mm なので、内側ガイドにするとロッドを
φ9.5 まで太らせることになる（ガイド径 ＝ ばね内径 − 1.0〜1.5 mm）。φ9.5 の管は
外側ガイド管より重い。外側なら管 1 本で「ばねの座屈防止」と「足のこじり防止」を兼ねられる。

**CAD で確認すること**: 脚をいちばん縮めて 40 mm 振ったとき（|OF| = 139）、
ロッド上端が股ブッシュより **73 mm** 上に出る。
...
50 g を 3.2 g 超える。削るならシャンク（PA-CF 中空の肉厚 1.0 → 0.8）かキャリアの肉抜き。
ばねは規格品なので動かせない。
