...
## 11. 参考: 既存の Fusion 360 アドイン

`tools/fusion/QuadLegLinkage/` は四脚の四節リンク用だが、**既定サーボが XM540-W270 で書かれている**ため流用できる。
5 節リンク＋ロール軸＋直列バネの骨格・サーボ本体・ジョイントを生成する版に拡張できる（要望があれば作る）。
座標系は「股関節 O が原点、+X 前方、+Y 上、リンクは XY 平面、Z 方向に板厚」。
