...
### 4-6. 脚長サーボは脚面を挟んで向かい合わせに置く（2026-09-20 確定）

**サーボ① は脚面の +Z 側にホーンを −Z 向きで、サーボ② は −Z 側にホーンを +Z 向きで置き、
2 本のクランクを同じ脚面（z = 0）に載せる。**

```
z:  −49.1 ─────────── −5.1  −2.5   0   +2.5  +5.1 ─────────── +49.1
     サーボ② 本体 44      ホーン②  クランク①②   ホーン①   サーボ① 本体 44
                                  （脚面）
```

クランク① は可動域で常に x < −44、クランク② は x > +44 に留まる（§6）ので、
**2 本のクランクは同じ z に置いてよい**。ホーン②（x +7〜+33）とクランク①のハブ（x −33〜−7）も
XY で重ならない。

**なぜこの配置か** — 3 案を比較した（`hopper-log.md` §8.13）

| | 同一方向（両ホーンが同じ側） | 左右反転（脚面の外側で向き合う） | **向かい合わせ（脚面を挟む）** |
| --- | --- | --- | --- |
| 脚が平面か | ○ | × 下腿が 8.4° 傾く | **○** |
| サーボ出力軸のスラスト | 2.0 N | 17.3 N | **≈ 0** |
| 下腿の長さ・§5-1 §7-3 の層・ピン C | そのまま | 131.4 mm・引き直し・φ4 | **そのまま** |
| ブラケットの面外モーメント | 5664 N·mm | 0 | **0** |
| 機体重心の合わせ込み | 胴体を 34 mm 片側へ寄せる | 不要 | **不要（脚面 = サーボ対の中心）** |
| 股の Z 方向の厚み | 約 61 mm | 約 93 mm | 約 98 mm |

- 同一方向は脚面がサーボ対の中心から 26 mm ずれる。§4-2 に従ってロール軸をそこへ置くと、
  機体重心も脚面に載せなければならず（載せないと接地力 × 26 mm ＝ 1.2〜3.0 N·m が
  ロールサーボの定常負荷になり不可）、胴体を片側へ寄せる設計要件が増える
- 左右反転は「ホーンの軸方向荷重を相殺する」意図だったが、平面の 5 節リンクにはもともと
  軸方向の力が無く、反転で下腿が傾くことでかえって 17.3 N のスラストが出る
- 向かい合わせは両者の利点（面外モーメント 0・重心が中央）を、脚を傾けずに得る。
  代償は股の厚みだけで、脚サーボ 2 個が z = ±27 に対称に載るためロール慣性の増分は
  2.4 × 10⁻⁴ kg·m²（モデルの 3 %）

**ホーンの片持ちは配置と無関係に残る。** 両持ちにするなら各クランクの反対面
（クランク① なら z −3〜−6、x −33〜−7）に軸受板を置く。ホーン②（x +7〜+33）とは重ならない。

**ロール軸**は脚面（z = 0）を通す必要があるが、y = 0 にはクランクのハブがある。
サーボの上端（y = +29）より上、**y ≥ +35 を通す**のが現実的（§10）。
...
- **足先 F から 30 mm の位置に φ4 H7 の穴を 1 つ追加**する（菱形リンクのピン A1 / A2。§7-2、2026-09-20 確定）。
  穴のまわりだけ**局所ボス φ9 × 外面へ +3.25 mm**を盛り、その位置の板厚を 2.5 → **5.75 mm**にする
  （圧入代の目安 1.5〜2×φ4 ＝ 6〜8 mm には届かないが、比 1.44×D は圧入として許容範囲。
  ボスの肉厚は (9−4)/2 = 2.5 mm で圧入時に割れない）。
  **+3.25 mm は §7-3 のスペーサと同じ寸法**にしてあるので、ボスの端面がそのまま小リンクの受け面になり、
  **別部品のスペーサが不要になる**（ボスが外面側の隙間をちょうど埋め、内面側＝もう一方のシャンクに
  向く 0.5 mm の隙間は侵さない。§4-6）
  **PA-CF プリント案ではこのボスにも金属ブッシュ（内径 φ4 / 外径 φ7 程度、無給油スリーブ）を
  圧入 or インサート成形してから、そこへ SUS ピンを圧入する**（樹脂へ直接圧入するとクリープで
  緩む。上の「(2) ピン穴に金属ブッシュを圧入」と同じ理由）。K・F と同じ考え方を A1・A2 にも適用する
...
**向かい合わせ配置（§4-6）での XY 最小距離**（可動域全体: 先端 ±40 mm・脚長 139〜199 mm。
同じ z 帯にあって衝突しうる組だけ。部品の幅 10〜13 mm を引いても余裕がある）
...
| シャンク① ↔ サーボ① 本体 | 54.8 mm |
| シャンク② ↔ サーボ② 本体 | 54.8 mm |
| クランク① ↔ クランク②（同じ z） | 40.0 mm |

**確認すべき干渉**

1. クランク① ↔ サーボ②本体、クランク② ↔ サーボ①本体（軸間 40 mm、本体幅 33.5 mm。上表で 23 mm）
2. 下腿どうし（内角 55° の伸展姿勢）
...
![菱形リンクの部品図](img/hopper-bisector-parts.png)

（機構の図は `tools/make_bisector_diagram.py`、部品図は `tools/make_link_parts_diagram.py` が生成する）
...
内角が変わると C は二等分線上を F から **42.2〜53.1 mm** の範囲で動く（可動域全体で実測。
内角 90.5°→55.5°）。スロットはその逃げで、丸端の中心を 40 / 55 mm に置けば両端 2 mm の余裕になる。

**菱形の性質**: 対辺は平行なので、**リンク① (A1→C) は常にシャンク② と平行**、リンク② はシャンク① と平行。
CAD で角度を測れば検算できる（全姿勢で差 0.000°）。
...
| 部品 | 仕様 | 備考 |
| --- | --- | --- |
| 足先ブロック | F の二股（§5-1）＋**腕 × 2**（幅 8 × 厚 2.5、F から上へ **60 mm**、**スロット 幅 3 × 全長 18**：丸端の中心を F から **40 mm と 55 mm** に）＋ガイド管、を 1 部品 | **二股の耳を両方とも上へ延ばす**（z = ±4.75〜±7.25）。1 枚だとピン C の偶力を受けられない |
| **小リンク × 2** | ピン間 **30 ±0.05 mm**、幅 8（両端 R4）、A2017 **t2**、**φ4 H7 × 2** | 各シャンクの外面側の層。二力部材（軸力 140 N → 引張 17.5 MPa、面圧 17.5 MPa）。**1.54 g × 2** |
| **ピン A1・A2** | **φ4 h6 圧入ピン**（SUS303、ねじ無し）。シャンクの局所ボスに **H7/n6 で圧入**（ブラインド、貫通させない） | 小リンク側は φ4 の段を通してすきま嵌め＋**E リングで抜け止め**。**別部品のスペーサは不要**（ボスの端面が兼ねる。§5-2） |
| ピン C | φ3 × 22、両端 E リング | リンク①・腕①・腕②・リンク② を貫く。締め付けない |
...
| --- | --- |
| −9.5 〜 −7.5 | リンク① |
| −7.25 〜 −4.75 | 腕①（足先ブロックの耳①を延長。スロット） |
| −4.25 〜 −0.25 | シャンク① |
| +0.25 〜 +4.25 | シャンク② |
| +4.75 〜 +7.25 | 腕②（耳②を延長。スロット） |
| +7.5 〜 +9.5 | リンク② |

**腕は必ず 2 枚にする。** 1 枚だと、リンク①（z −8.5）とリンク②（z +8.5）が出す力の直角成分が
14 mm 離れた逆向きの対になり、**偶力 1386 N·mm が残る**。1 点では偶力を支えられないので静定しない。
2 枚にすればリンク①を腕①（z −6）が、リンク②を腕②（z +6）が受け、スパン 2.5 mm・
ピン C の曲げ 93 MPa（SF 2.2）に収まる。

**A1・A2 を圧入ピンにした理由**（2026-09-20 確定。`hopper-log.md` §8.14）

段付ボルトのねじ込みで固定する案も検討したが、シャンク側の板厚が 2.5〜4 mm しかなく、
M2〜M3 クラスのねじではかかり代が不足し、跳躍の繰り返し衝撃で緩む懸念があった。
薄板への軸取り付けは一般に「**穴のまわりだけ局所的に肉厚化してから固定**」するのが定石
（板金のバーリング加工、PEM 等のセルフクリンチングファスナーも同じ考え方）で、
ねじではなく**圧入**にすればそもそも緩む機構がなくなる。そこで A1・A2 は

- 段部を **φ3 → φ4** に太くする（軸力 140 N に対する曲げ安全率が 1.6〜2.0 → 3.9〜4.7 に改善。
  ついでに膝 K・足先 F と同径になり、φ4 用の工具・治具を共用できる）
- シャンク側に**局所ボス**（φ9 × +3.25 mm）を盛って板厚を 5.75 mm に確保し、そこへ**圧入**（H7/n6）
- 小リンク側は φ4 のすきま嵌め＋**E リング**で抜け止め（ねじ・ナットは使わない）

に変更した。ボスの高さを元のスペーサ寸法（3.25 mm）に合わせたので、**別部品のスペーサが不要**になる副産物もある。
なお、膝 K・足先 F の爪（t2.5〜3 mm）も段付ボルトのねじ込み深さという点では同種の懸念があり、
同じ考え方（局所ボス＋圧入）を適用する余地がある。今回は A1・A2 のみ変更し、K・F は §5-1 のまま
（**未決事項として §10 に記載**）。
...
4. 腕①・腕② の間（z −4.75 〜 +4.75）をピン C が無支持で渡る。ここに他部品が入らないこと
...
| `DEFLECTION` | 高さ | 有効部ピッチ | コイル隙間 | 荷重 | 用途 |
| --- | --- | --- | --- | --- | --- |
| 0 | 100.0 mm | 4.396 | 2.796 mm | 0 N | 組付け前・部品図 |
| 9.4 | 90.6 mm | 3.961 | 2.361 mm | 14.7 N | 静止立位 |
| 44.2 | 55.8 mm | 2.351 | 0.751 mm | 69.0 N | 通常ホップ 5 cm |
| 47.5 | 52.5 mm | 2.199 | 0.599 mm | 74.1 N | 落下 10 cm（常用の最大） |
| 55.8 | 44.2 mm | 1.815 | 0.215 mm | 87.0 N | 落下 15 cm（δ の最大） |
| 58.0 | 42.0 mm | 1.713 | 0.113 mm | 90.5 N | 底付きストッパ位置 |

死巻（上下 1 巻）にも線径ぶんのピッチ（1.68 mm）を持たせてある。ピッチ 0 の完全に平らな巻きに
すると同じ位置を 2 周することになり、掃引したソリッドが自己交差して Fusion が失敗する
（`ASM_SELF_INTER`）。そのため**実物の研削端の平らさは再現していない**が、質量と干渉には影響しない。
この形状で作れる高さの下限は 41.3 mm（δ 58.7 mm）で、密着長 37.8 mm までは作れない。
使用範囲（δ ≤ 58）はすべて作れる。
...
| 部品 | 質量 |
| --- | --- |
| シャンク 2 本（PA-CF 中空 ＋ 金具 ＋ A1/A2 局所ボス 2 箇所 ＋ 金属ブッシュ 2 個） | 16.8 g |
| キャリア（足先ブロック） | 5.0 g |
...
| ゴムパッド | 3.0 g |
| ピン・軸受類 | 5.0 g |
| 菱形リンク（小リンク 1.54×2 ＋ ピン C（φ3、両端Eリング）1.23 ＋ ピン A1/A2（φ4 圧入、片端Eリング）1.19×2 ＋ E リング 0.2） | 6.9 g |
| 足先ブロックの腕 × 2（60 × 8 × t2.5、スロット付き） | 2.6 g |
| **合計** | **64.5 g** |

50 g を 14.5 g 超える。削るならシャンク（PA-CF 中空の肉厚 1.0 → 0.8）か足先ブロックの肉抜き。
ばねは規格品なので動かせない。なお旧方式のロッド（11.6 g）と股ブッシュブロック（約 10 g）は
この表の外（脚の質量）だったので、機体全体では約 17 g 軽くなっている
（A1/A2 を圧入ピン＋局所ボスにした増分 約 2 g を差し引いても、なお軽い）。
...
1. **ロールサーボの逃がし方**（§4-2、§4-6）。前後にずらすと脚の振り角と干渉する。
   脚面の y = 0 にはクランクのハブがあるので、ロール軸は **y ≥ +35（サーボ上端より上）**を通す案が有力。
   ブラケットは向かい合わせの 2 個のサーボを z の反対側に保持する（コの字か S 字）。CAD の主要な検討点
...
7. **膝 K・足先 F の段付ボルトも圧入化するか**（§7-3）。A1・A2 と同じ薄板（爪 t2.5〜3 mm）に
   ねじ込む構造で、緩みの懸念は同種。分解・調整のしやすさ（現行の段付ボルト）と
   緩み耐性（圧入化）のどちらを優先するか、試作の結果を見てから判断する
...
サーボ① は脚面の +Z 側（ホーン −Z 向き）、サーボ② は −Z 側（ホーン +Z 向き）に置く（§4-6）。
アドインの層は「クランク → 下腿① → 下腿② → バネ」を Z に積む簡略版で、§5-1 の二股（下腿がクランクを挟む）は再現していない。
