...
```
O（股軸）── ロッドをここでピン支持（回転自由）
│
│  ガイドロッド（φ4〜5、長さ約 320 mm）
│
●  F ── キャリア＝足先ブロック（リニアブッシュ）。ロッド上を 125〜199 mm 滑る
┃
≋  圧縮コイルばね（ロッドに巻く）。上側ばね座＝キャリア下面
▬  下側ばね座（ロッドに固定するカラー）
●  足＋ゴムパッド（ロッド下端）
```

**シャンクにバネを直付けしてはいけない。** 理由は 3 つある。
...
3. **ロッドを股で支持すれば向きは自動的に決まる。**
   支点 O と通過点 F が決まればロッドの角度は一意で、角度を決める機構が要らない。
...
| 部品 | 仕様 | 備考 |
| --- | --- | --- |
| ガイドロッド | φ4〜5、長さ **約 320 mm** | 上端に φ3 ピン穴。F より上は軸力をほぼ受けない |
| 股のピン支持 | φ3 段付ボルト＋ブッシュ | P1・P2 の間（40 mm の隙間）にブラケット |
| キャリア（＝足先ブロック） | リニアブッシュ＋シャンク 2 本のピン穴 F | 下面が上側ばね座（§5-1 参照） |
| 圧縮コイルばね | ロッドに巻く | 下記 7-4 |
| 下側ばね座 | セットカラーまたは E リング | |
| 底付きストッパ | ロッド上のカラー | 使用ストロークの 85 % で当てる |
| 足 | ロッド下端＋ゴムパッド | §4-4 |

ロッド長の内訳: 股→足先 F が最大 199 mm（先端 ±40 mm・伸び 195 mm のとき）＋バネの自由長。
